...
### Build-flag matrix (env vars)

Optional subsystems are **gated behind environment variables** so the default
`build/kernel.elf` stays byte-for-byte stable and risky/hardware-specific code only
compiles in when you ask for it. Set the var to `1` in front of the build command.
Kernel-side flags go to `quick_build.sh`; init/userspace flags go to `build_all.sh`.
...
| `HDA_ENABLE=1` | `quick_build.sh` | Compile in **Intel HD Audio** init (codec via the Immediate Command Interface, DMA stream playback). QEMU-safe; T410-unsafe. Required by the Sound Manager + `SYS_AUDIO_*`. |
...
| `PREEMPT=1` | `quick_build.sh` | Gated **preemptive scheduler** (`-DPREEMPTIVE`) -> `build/kernel-preempt.elf`. Default scheduler is cooperative. |
| `IDE=1` | `build_all.sh` | init auto-opens the Semantic LEGO Map IDE (`-DIDE_AUTOSTART`). |
| `GAMETEST=1` | `build_all.sh` | init spawns `sbin/gametest` (`-DGAMETEST_RUN`), the 25-app spawn+survive harness (see the smoke section). |
...
| `DESKTOP_MINIMAL=1` | `build_all.sh` | init spawns only the persistent desktop apps and skips the self-test storm — faster, quieter boot. |
...
Example — a documentation/demo image with simulated WiFi, sound and the showcase apps:
...
supports — ARP/DNS/TCP/HTTPS all work in QEMU. Add `-smp 2` when running an `SMP=1`
kernel. QEMU has no WiFi radio, so the **real** iwlwifi (`IWLWIFI=1`) RF tail can only be
exercised on the T410; use the simulated backend (`WIFI_SIM=1`) to drive the WiFi
control plane and Network Manager in QEMU.
...
desktop, spawns processes, brings up networking, renders the browser, runs the on-device
compiler, exercises the crypto/TLS/web stack, etc. A green run is **43/43** — the
project's gate.

#### The game/app harness (`GAMETEST=1`)

A separate harness proves the apps actually launch and stay alive. Build the image with
`GAMETEST=1` (see the flag matrix) and init spawns `sbin/gametest`, which spawns each of
the **25** bundled games + apps in turn, waits, and confirms it is still running:
...
A green run is **25/25** (snake, tetris, game2048, mines, breakout, pong, invaders,
solitaire, chess, asteroids, sudoku, pacman, zombietd, cube3d, ray, calculator,
clockapp, settings, paint, notes, sheet, taskman, filemanager, bubbletd, photos).
...
The real Intel iwlwifi radio has **no emulator** — its RF bring-up is iterated directly
on the physical T410. Build with `IWLWIFI=1`, boot to the desktop, then run `sbin/iwlup`
(it never runs at boot) to attach + start the card; the Network Manager shows a live
`Radio:` diagnostics line (`SYS_WLAN_DIAG`). The boot KATs (IWL-RXON / IWL-SCAN /
IWL-FWSEL / IWL-FW) pass in QEMU, but association and the data plane are the **next
milestones** on hardware. The full hardware loop — firmware staging, RF-kill, the
diagnostic line, and what works vs. what is still being iterated — is documented in
[docs/T410_IWLWIFI.md](../T410_IWLWIFI.md).
