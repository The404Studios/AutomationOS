...
Welcome to the documentation for **AutomationOS** — a from-scratch x86_64 operating system that boots to a Windows-like graphical desktop on **QEMU and real hardware** (a 2010 Lenovo ThinkPad T410). Every layer is original code: the kernel, the compositor, the web browser engine, the C compiler, the TLS stack, the WiFi driver, and a gated AI agent that can drive the machine.

![AutomationOS desktop](../../screenshots/desktop.png)

## Pages

Start here and follow the layer you care about, from boot up through the desktop.
...
A quick look at the pieces in action, each linked to its deep-dive page.
...
| ![Network Manager](../../screenshots/netman_diag.png) | **WiFi + Network Manager** — scan/connect plus a live "Radio:" bring-up diagnostics line |
| ![App suite](../../screenshots/showcase.png) | **The app suite** — IDE, cockpit, games and more, all running at once |

## What makes it interesting

The parts that set AutomationOS apart from a typical toy kernel.
...
- **From-scratch browser**: its own DOM, HTML/CSS/layout, and a JavaScript engine, over **TLS 1.2 & 1.3**.
- **A from-scratch Intel WiFi driver** (iwlwifi DVM) for the T410 — firmware auto-select plus on-screen bring-up diagnostics, with a full WPA2 + **WPA3-SAE** supplicant.
- **A gated AI agent** that can drive the OS (mouse/keyboard/shell/files) through a capability-gated, audited (tamper-evident ledger), human-supervised cockpit.
- **A real desktop**: a compositor with a dock, window snap/maximize, a Windows-11 Start menu, HDA sound, 25+ apps and games, a circular boot transition, and eased animations.

## Quick start

Build and boot-test the system under WSL Arch Linux.
...
AutomationOS is a hobby OS and a work in progress; here is what that means in practice.

- The scheduler is **cooperative** by default (gated `PREEMPT=1` and `SMP=1` builds exist and are validated).
- It boots via GRUB Multiboot on legacy BIOS, not UEFI.
- The from-scratch **iwlwifi** WiFi driver's logic is known-answer-proven in QEMU, but actual radio bring-up on the physical T410 (which has no emulator) is iterated on hardware via the on-screen diagnostics; WiFi association and the data plane are the next milestones.
