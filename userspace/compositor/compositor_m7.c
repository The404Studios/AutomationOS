/*
 * compositor_m7.c -- Milestone M7 WINDOW MANAGER (ring 3): m6 + macOS dock.
 *
 * Copied verbatim from compositor_m6.c. m6 is preserved byte-for-byte in
 * behavior. M7 ADDS a macOS-style dock with hover magnification, app folders,
 * launch bounce, and animated folder popovers above the existing taskbar strip.
 *
 * PRESERVED FROM M6 (all intact, not modified):
 *   - Window snapping (left/right/max) with animated geometry tween + preview
 *   - Alt+Tab MRU focus cycling
 *   - Alt+Q / Alt+F4 close, Alt+M minimize, Alt+K FORCE QUIT (SYS_KILL SIGKILL)
 *   - reap_dead_windows() per-frame kill(pid,0) liveness sweep (every 30 frames)
 *   - Notification toasts (fade-in/hold/fade-out)
 *   - Open/close/minimize/restore animations (fixed-point, no float/SSE)
 *   - Rounded corners, soft layered drop shadows, vertical gradient wallpaper
 *   - Top panel with clock + focused window title
 *   - Bottom dock with launcher button + per-window taskbar buttons
 *   - Zero-copy SHM client compositing (blit_surface_clip / scaled_alpha)
 *   - Keyboard + pointer forwarding to focused window
 *
 * NEW IN M7 -- macOS-style dock
 * ==============================
 *  1. DOCK ICON STRIP: 16 app icons (rounded-square tiles, 48px base) drawn
 *     centered at the bottom, above the existing taskbar. Each tile shows a
 *     unique color + the app's initial letter(s) via bitfont.
 *
 *  2. HOVER MAGNIFICATION (genie effect): scale = 1 + (MAX_SCALE-1) *
 *     max(0, 1 - dist/INFLUENCE). MAX_SCALE=2.0 (Q8=512), INFLUENCE=120px.
 *     Each icon's rendered size = base * scale_q8 / 256. Neighboring icons
 *     scale with falloff. Icons "lift" upward as they magnify (y -= extra_h/2).
 *     scale_cur interpolates toward scale_tgt each frame (ease) for fluid feel.
 *
 *  3. LAUNCH BOUNCE: clicking an icon SYS_SPAWNs the app and the icon bounces
 *     on a damped vertical sine for ~600ms (8 frames per period, 3 periods,
 *     amplitude decays by 0.6 per period). All via fixed-point integer math.
 *
 *  4. APP FOLDERS / CATEGORY BOXES: hard-coded Groups (Games: Snake/Tetris/2048;
 *     Tools: Calculator/Clock/Editor/Paint; Media: Synth/Dashboard). A folder
 *     icon renders as a 2x2 mini-grid of its members. Clicking expands a fan/grid
 *     popover above the dock. Clicking a popover member launches it. Clicking
 *     elsewhere collapses the popover. Expand/collapse animated (scale 0->1).
 *
 * Build:
 *   gcc -std=gnu11 -ffreestanding -nostdlib -fno-builtin -fno-stack-protector \
 *       -fno-pic -fno-pie -mno-red-zone -O2 \
 *       -c userspace/compositor/compositor_m7.c -o /tmp/cm7.o
 *   gcc <same flags> -c userspace/lib/font/bitfont.c -o /tmp/bf.o
 *   ld -nostdlib -static -n -no-pie -e _start -T userspace/userspace.ld \
 *       /tmp/cm7.o /tmp/bf.o -o /tmp/cm7.elf
 *   objdump -d /tmp/cm7.elf | grep "fs:0x28"   # MUST be empty
 *
 * (Below: preserved m6 comment block, kept for historical context.)
 *
...
/* chrome geometry */
#define PANEL_H     28
/* RIGHT-SIDE DOCK (M7 redesign): vertical strip on right edge */
#define DOCK_W      68   /* dock strip width on the right edge               */
/* DOCK_H kept for backward compat in a few clip calculations; set to 0 so
 * windows are no longer clipped at the bottom for a dock bar. */
#define DOCK_H      0
#define CLOSE_SZ    14                 /* titlebar close box hit/visual size  */
#define MIN_SZ      14                 /* titlebar minimize box hit/visual    */
/* Taskbar constants kept for animate-to-taskbar code; taskbar itself removed. */
#define TASK_W      120
#define TASK_H      32
#define LAUNCH_SZ   36
...
/* ---- framebuffer dimensions: declared here so dock code (below) can use them.
 * Initialised to 0 at startup; set during _start after SYS_FB_ACQUIRE. ---- */
...
/* Dock icon geometry tunables -- RIGHT-SIDE VERTICAL DOCK */
#define DOCK_ICON_BASE   48   /* base icon tile size (px)                   */
#define DOCK_ICON_GAP    6    /* gap between icon tiles (vertical)           */
#define DOCK_ICON_RADIUS 10   /* rounded-square corner radius               */
#define DOCK_MAG_MAX_Q8  512  /* MAX_SCALE = 2.0 in Q8 (256 = 1.0)          */
#define DOCK_INFLUENCE   120  /* magnification falloff radius (px, Y-axis)  */
#define DOCK_BOUNCE_MS   600  /* total bounce animation duration             */
#define DOCK_LABEL_H     20   /* label tooltip height (used for popover)    */
/* X inset of icon tiles from dock left edge */
#define DOCK_STRIP_INSET 4
/* Horizontal left-bounce amplitude: icon slides left when launched */
#define DOCK_BOUNCE_AMP  14

/* Number of loose app icons + folder icons rendered on the dock bar */
#define DOCK_MAX_ITEMS   24   /* loose icons + folder slots combined         */

/* App list: all 16 apps as (label, spawn-path, tile-color) */
#define DOCK_NLOOSE_APPS 16

typedef struct {
    const char *label;     /* display name                              */
    const char *path;      /* relative spawn path for SYS_SPAWN         */
    uint32_t    color;     /* tile background color (opaque ARGB)        */
} dock_app_t;

/* 16 apps as specified; colors chosen to be visually distinct */
static const dock_app_t k_dock_apps[DOCK_NLOOSE_APPS] = {
    { "Terminal",   "sbin/terminal",   0xFF2D7D46u },  /* green          */
    { "Files",      "sbin/filemanager",0xFF1A73E8u },  /* blue           */
    { "Calculator", "sbin/calculator", 0xFF6B3FA0u },  /* purple         */
    { "Clock",      "sbin/clock",      0xFFB5460Fu },  /* burnt orange   */
    { "Editor",     "sbin/editor",     0xFF0A6B6Bu },  /* teal           */
    { "Paint",      "sbin/paint",      0xFFCC3333u },  /* red            */
    { "Snake",      "sbin/snake",      0xFF338033u },  /* dark green     */
    { "Tetris",     "sbin/tetris",     0xFF1A5276u },  /* navy           */
    { "2048",       "sbin/game2048",   0xFFCC7A00u },  /* amber          */
    { "Sheet",      "sbin/sheet",      0xFF0E6655u },  /* dark teal      */
    { "Notes",      "sbin/notes",      0xFF7D6608u },  /* ochre          */
    { "Calendar",   "sbin/calendar",   0xFF4A235Au },  /* dark violet    */
    { "Dashboard",  "sbin/dashboard",  0xFF1F618Du },  /* steel blue     */
    { "Synth",      "sbin/synth",      0xFF76448Au },  /* medium purple  */
    { "Settings",   "sbin/settings",   0xFF515A5Au },  /* dark slate     */
    { "Date",       "sbin/dateapp",    0xFF943126u },  /* deep red       */
};

/* Folder definition: a named group of app indices into k_dock_apps[] */
#define DOCK_MAX_FOLDER_MEMBERS 6
#define DOCK_NFOLDERS 3

typedef struct {
    const char *label;
    int         count;
    int         members[DOCK_MAX_FOLDER_MEMBERS];  /* indices into k_dock_apps */
    uint32_t    color;
} dock_folder_def_t;

/* Hard-coded folders: Games (6,7,8), Tools (2,3,4,5), Media (13,12) */
static const dock_folder_def_t k_folders[DOCK_NFOLDERS] = {
    { "Games",  3, { 6, 7, 8,  0, 0, 0 }, 0xFF1A5276u },
    { "Tools",  4, { 2, 3, 4, 5, 0, 0 }, 0xFF6B3FA0u },
    { "Media",  2, { 13, 12, 0, 0, 0, 0 }, 0xFF76448Au },
};

/* Which loose apps are IN a folder (bit-mask per app index) -- computed once */
static int g_dock_in_folder[DOCK_NLOOSE_APPS]; /* 1 = belongs to a folder */
static int g_dock_folder_of[DOCK_NLOOSE_APPS]; /* folder index or -1      */

/* A dock "item" is either a loose app or a folder */
#define DITEM_APP    0
#define DITEM_FOLDER 1

typedef struct {
    int  kind;         /* DITEM_APP or DITEM_FOLDER                       */
    int  idx;          /* index into k_dock_apps[] or k_folders[]          */
} dock_item_t;

static dock_item_t g_dock_items[DOCK_MAX_ITEMS];
static int         g_dock_nItems = 0;

/* Per-item animated magnification scale (Q8, 256 = 1.0, target + current) */
static int32_t g_dock_scale_tgt[DOCK_MAX_ITEMS];  /* target scale Q8      */
static int32_t g_dock_scale_cur[DOCK_MAX_ITEMS];  /* current (smoothed)   */

/* Per-item bounce state */
typedef struct {
    long    start_ms;   /* when bounce began; 0 = not bouncing            */
    int     active;
} dock_bounce_t;
static dock_bounce_t g_dock_bounce[DOCK_MAX_ITEMS];

/* Expanded folder popover state */
static int   g_folder_open    = -1;   /* index into g_dock_items, or -1   */
static int32_t g_popover_anim = 0;    /* Q8 open progress 0..256          */
static long    g_popover_anim_start = 0;
#define POPOVER_ANIM_MS 160

/* Cached rendered icon rect (magnified), filled each frame during render */
typedef struct {
    int32_t x, y, w, h;   /* screen rect of the rendered icon tile        */
} dock_icon_rect_t;
static dock_icon_rect_t g_dock_rects[DOCK_MAX_ITEMS];

/* ------------------------------------------------------------------ *
 *  M7: dock initialiser -- build the item list from apps + folders   *
 * ------------------------------------------------------------------ */
static void dock_init(void) {
    /* Mark which apps are in folders */
    for (int i = 0; i < DOCK_NLOOSE_APPS; i++) {
        g_dock_in_folder[i] = 0;
        g_dock_folder_of[i] = -1;
    }
    for (int f = 0; f < DOCK_NFOLDERS; f++) {
        for (int m = 0; m < k_folders[f].count; m++) {
            int ai = k_folders[f].members[m];
            if (ai >= 0 && ai < DOCK_NLOOSE_APPS) {
                g_dock_in_folder[ai] = 1;
                g_dock_folder_of[ai] = f;
            }
        }
    }
    ...
    /* First: loose apps (not in any folder) */
    for (int i = 0; i < DOCK_NLOOSE_APPS; i++) {
        if (g_dock_in_folder[i]) continue;
        if (g_dock_nItems >= DOCK_MAX_ITEMS) break;
        g_dock_items[g_dock_nItems].kind = DITEM_APP;
        g_dock_items[g_dock_nItems].idx  = i;
        g_dock_nItems++;
    }
    ...
    for (int f = 0; f < DOCK_NFOLDERS; f++) {
        if (g_dock_nItems >= DOCK_MAX_ITEMS) break;
        g_dock_items[g_dock_nItems].kind = DITEM_FOLDER;
        g_dock_items[g_dock_nItems].idx  = f;
        g_dock_nItems++;
    }

    /* Init scale and bounce */
    for (int i = 0; i < g_dock_nItems; i++) {
        g_dock_scale_tgt[i] = 256;
        g_dock_scale_cur[i] = 256;
        g_dock_bounce[i].active   = 0;
        g_dock_bounce[i].start_ms = 0;
    }

    print("[SHELL] M7 dock: ");
    print_num(g_dock_nItems);
    print(" icons, ");
    print_num(DOCK_NFOLDERS);
    print(" folders\n");
}

/* ------------------------------------------------------------------ *
 *  M7: fixed-point integer sqrt approximation (Newton, 5 iters)      *
 *  Input is 32-bit, result fits in 16 bits. Avoids all float/div64.  *
 * ------------------------------------------------------------------ */
static int32_t isqrt32(int32_t n) {
    if (n <= 0) return 0;
    int32_t x = n;
    /* crude starting guess: shift right to ~half the bits */
    int32_t s = x;
    x = 1;
    while (s > 1) { s >>= 2; x <<= 1; }
    /* 6 Newton iterations: x = (x + n/x) / 2 */
    x = (x + n / (x ? x : 1)) / 2;
    x = (x + n / (x ? x : 1)) / 2;
    x = (x + n / (x ? x : 1)) / 2;
    x = (x + n / (x ? x : 1)) / 2;
...
/* ------------------------------------------------------------------ *
 *  M7: update per-item magnification scales based on cursor position  *
 *  and smooth them toward target.  Called each frame before render.  *
 * ------------------------------------------------------------------ */
static void dock_update_magnification(int32_t cx, int32_t cy) {
    for (int i = 0; i < g_dock_nItems; i++) {
        /* center of the base (unmagnified) icon's current rect */
        int32_t ix = g_dock_rects[i].x + g_dock_rects[i].w / 2;
        int32_t iy = g_dock_rects[i].y + g_dock_rects[i].h / 2;
        /* horizontal distance only (like macOS dock) */
        int32_t dist = cx - ix;
        if (dist < 0) dist = -dist;
        /* scale = 256 + (DOCK_MAG_MAX_Q8-256) * max(0, 1 - dist/INFLUENCE)
         * In integer: factor = max(0, (INFLUENCE - dist) * 256 / INFLUENCE) */
        int32_t tgt = 256;
        if (dist < DOCK_INFLUENCE) {
            /* Only magnify when cursor is over the dock strip */
            if (cy >= (int32_t)g_fb_h - DOCK_H - 8) {
                int32_t factor = (DOCK_INFLUENCE - dist) * 256 / DOCK_INFLUENCE;
                tgt = 256 + (int32_t)(((int64_t)(DOCK_MAG_MAX_Q8 - 256) * factor) >> 8);
                if (tgt > DOCK_MAG_MAX_Q8) tgt = DOCK_MAG_MAX_Q8;
            }
        }
        g_dock_scale_tgt[i] = tgt;
        /* Smooth interpolation: cur += (tgt - cur) * 6 / 16 per frame */
        int32_t diff = tgt - g_dock_scale_cur[i];
        g_dock_scale_cur[i] += (diff * 6) >> 4;
        if (g_dock_scale_cur[i] < 256) g_dock_scale_cur[i] = 256;
        if (g_dock_scale_cur[i] > DOCK_MAG_MAX_Q8) g_dock_scale_cur[i] = DOCK_MAG_MAX_Q8;
    }
}

/* ------------------------------------------------------------------ *
 *  M7: bounce offset for an item at time `now`. Returns upward pixel  *
 *  offset (positive = move up). Fixed-point damped sine via integer.  *
 * ------------------------------------------------------------------ */
static int32_t dock_bounce_offset(int item, long now) {
    if (!g_dock_bounce[item].active) return 0;
    long dt = now - g_dock_bounce[item].start_ms;
    if (dt < 0) dt = 0;
    if (dt >= DOCK_BOUNCE_MS) {
        g_dock_bounce[item].active = 0;
        return 0;
    }
    /* 3 periods over DOCK_BOUNCE_MS; period = DOCK_BOUNCE_MS / 3 */
    long period = DOCK_BOUNCE_MS / 3;
    int  which_period = (int)(dt / period);          /* 0,1,2         */
    long t_in_period  = dt - (long)which_period * period;
    /* sine approximation via parabola: x in [0,period] -> 0..1..0
     * val_q8 = 256 * 4 * t * (period - t) / (period * period) */
    long num   = 4L * t_in_period * (period - t_in_period);
    long den   = period * period / 256;
    int32_t sin_q8 = (den > 0) ? (int32_t)(num / den) : 0;
    if (sin_q8 < 0) sin_q8 = 0;
    if (sin_q8 > 256) sin_q8 = 256;
    /* amplitude decays: period 0 -> 12px, 1 -> 7px, 2 -> 4px */
    static const int32_t amp[3] = { 12, 7, 4 };
    int32_t a = (which_period < 3) ? amp[which_period] : 0;
    return (a * sin_q8) >> 8;
}

/* ------------------------------------------------------------------ *
 *  M7: render a single dock icon tile at (tx,ty) with size sz.        *
 *  kind/idx identify the item for color + label.                      *
 * ------------------------------------------------------------------ */
static void render_dock_tile(uint32_t *buf, uint32_t bw, uint32_t bh, uint32_t stride,
                             int32_t tx, int32_t ty, int32_t sz,
                             int kind, int idx, int hovered) {
    uint32_t color;
    const char *label;
    if (kind == DITEM_APP) {
        color = k_dock_apps[idx].color;
        label = k_dock_apps[idx].label;
    } else {
        color = k_folders[idx].color;
        label = k_folders[idx].label;
    }

    /* Subtle hover highlight: brighten the tile slightly */
    if (hovered) {
        uint32_t r = ((color >> 16) & 0xFF); r = r + (255 - r) / 3;
        uint32_t g2 = ((color >> 8) & 0xFF);  g2 = g2 + (255 - g2) / 3;
        uint32_t b = (color & 0xFF);          b = b + (255 - b) / 3;
        color = 0xFF000000u | (r << 16) | (g2 << 8) | b;
    }

    if (kind == DITEM_FOLDER && k_folders[idx].count >= 2) {
        /* Folder: draw a 2x2 mini-grid of the first 4 members' colors */
        int32_t half = sz / 2 - 1;
        fill_round_rect(buf, bw, bh, stride, tx, ty, sz, sz, DOCK_ICON_RADIUS, 0xFF3A3A3Cu);
        for (int m = 0; m < 4 && m < k_folders[idx].count; m++) {
            int ai = k_folders[idx].members[m];
            uint32_t mc = (ai >= 0 && ai < DOCK_NLOOSE_APPS) ? k_dock_apps[ai].color : 0xFF444444u;
            int32_t mx = tx + (m % 2) * (half + 1) + 2;
            int32_t my = ty + (m / 2) * (half + 1) + 2;
            int32_t mw = half - 2, mh = half - 2;
            if (mw < 4) mw = 4;
            if (mh < 4) mh = 4;
            fill_round_rect(buf, bw, bh, stride, mx, my, mw, mh, 3, mc);
        }
    } else {
        /* Single app tile: solid rounded square + 1-2 char initial */
        fill_round_rect(buf, bw, bh, stride, tx, ty, sz, sz, DOCK_ICON_RADIUS, color);
        /* Draw first 2 chars of label, centered */
        char init[3] = { label[0], (label[1] ? label[1] : '\0'), '\0' };
        int chars = (label[1] && sz >= 24) ? 2 : 1;
        init[chars] = '\0';
        int32_t lx = tx + (sz - chars * FONT_W) / 2;
        int32_t ly = ty + (sz - FONT_H) / 2;
        font_draw_string(buf, (int)stride, (int)bw, (int)bh, lx, ly, init, COL_TEXT);
    }
    /* Folder open indicator: small accent dot at bottom */
    if (kind == DITEM_FOLDER && g_folder_open >= 0) {
        /* (checked by caller) */
    }
    (void)hovered;
}
...
/* ------------------------------------------------------------------ *
 *  M7: render folder popover (fan/grid) above dock_item i.           *
 * ------------------------------------------------------------------ */
static void render_folder_popover(uint32_t *buf, uint32_t bw, uint32_t bh, uint32_t stride,
                                  int item_idx, int32_t icon_cx, int32_t icon_top,
                                  int32_t anim_q8, long now) {
    (void)now;
    if (item_idx < 0 || item_idx >= g_dock_nItems) return;
    if (g_dock_items[item_idx].kind != DITEM_FOLDER) return;
    int fidx = g_dock_items[item_idx].idx;
    if (fidx < 0 || fidx >= DOCK_NFOLDERS) return;

    int nmembers = k_folders[fidx].count;
    if (nmembers <= 0) return;

    /* Layout: grid of member tiles, 3 per row */
    int cols = 3;
    int rows = (nmembers + cols - 1) / cols;
    int32_t cell = 52;
    int32_t pad  = 8;
    int32_t box_w = cols * (cell + pad) + pad;
    int32_t box_h = rows * (cell + pad) + pad;

    /* Right-side dock: popover opens to the LEFT of the icon.
     * icon_cx here is the LEFT edge of the icon tile;
     * icon_top here is the vertical center of the icon. */
    int32_t bx = icon_cx - box_w - 8;  /* to the LEFT of the icon */
    int32_t by = icon_top - box_h / 2; /* vertically centered on icon */
    if (bx < 4) bx = 4;
    if (by < PANEL_H + 4) by = PANEL_H + 4;
    if (by + box_h > (int32_t)bh - 4) by = (int32_t)bh - 4 - box_h;

    /* Animate: scale from right edge (nearest the dock) */
    if (anim_q8 < 256) {
        int32_t scale_x = (box_w * anim_q8) >> 8;
        if (scale_x < 4) scale_x = 4;
        bx = icon_cx - scale_x - 8;
        box_w = scale_x;
    }

    /* Background: translucent dark rounded box */
    blend_rect(buf, bw, bh, stride, bx + 3, by + 4, box_w, box_h, 0x60000000u);
    {
        int32_t r2 = 10;
        uint32_t bcol = 0xEE2C2C2Eu;
        fill_round_rect(buf, bw, bh, stride, bx, by, box_w, box_h, r2, bcol);
        stroke_rect(buf, bw, bh, stride, bx, by, box_w, box_h, 0xFF4A4A4Cu);
    }

    /* Folder title label */
    font_draw_string(buf, (int)stride, (int)bw, (int)bh,
                     bx + pad, by + 4, k_folders[fidx].label, COL_TEXT_DIM);

    /* Member tiles */
    for (int m = 0; m < nmembers; m++) {
        int col = m % cols;
        int row = m / cols;
        int32_t tx = bx + pad + col * (cell + pad);
        int32_t ty = by + FONT_H + 4 + row * (cell + pad);
        int ai = k_folders[fidx].members[m];
        if (ai < 0 || ai >= DOCK_NLOOSE_APPS) continue;
        /* clip to popover box */
        if (ty + cell > by + box_h) continue;
        render_dock_tile(buf, bw, bh, stride, tx, ty, cell,
                         DITEM_APP, ai, 0);
        /* mini label below tile */
        const char *lab = k_dock_apps[ai].label;
        int32_t lw = (int32_t)k_strlen(lab) * FONT_W;
        if (lw > cell) lw = cell;
        font_draw_string(buf, (int)stride, (int)bw, (int)bh,
                         tx + (cell - lw) / 2, ty + cell + 2,
                         lab, COL_TEXT_DIM);
    }
}

/* ------------------------------------------------------------------ *
 *  M7 (redesigned): render the full dock icon strip on the RIGHT edge.*
 *  Vertical layout: icons stacked top-to-bottom within the dock strip *
 *  (right DOCK_W pixels).  Hover magnification uses Y-axis distance.  *
 *  Popover fans LEFT of the dock strip.                               *
 *  Called from composite() after render_panel() and render_dock().    *
 * ------------------------------------------------------------------ */
static void render_dock_icons(uint32_t *buf, uint32_t bw, uint32_t bh, uint32_t stride,
                              int32_t cur_x, int32_t cur_y, long now) {
    /* [DBG] trace entry to isolate crash location */
    print("[DBG] rdi entered n="); print_num(g_dock_nItems);
    print(" bw="); print_num((long)bw); print(" bh="); print_num((long)bh); print("\n");
    if (g_dock_nItems == 0) return;

    /* Dock strip occupies the right DOCK_W pixels, from below the panel to the
     * bottom of the screen.  Icons are centered horizontally in the strip and
     * stacked vertically with DOCK_ICON_GAP between them. */
    int32_t dock_x0 = (int32_t)bw - DOCK_W;  /* left edge of the dock strip */
    int32_t icon_cx = dock_x0 + DOCK_W / 2;  /* horizontal center of strip  */

    /* Total height of all icons at base scale */
    int32_t total_base = g_dock_nItems * DOCK_ICON_BASE + (g_dock_nItems - 1) * DOCK_ICON_GAP;
    /* Top of icon column: centered vertically in the work area */
    int32_t avail_h  = (int32_t)bh - PANEL_H - 8;
    int32_t start_y;
    if (total_base < avail_h)
        start_y = PANEL_H + 8 + (avail_h - total_base) / 2;
    else
        start_y = PANEL_H + 8;

    /* First pass: compute unmagnified center-Y per item */
    int32_t centers_y[DOCK_MAX_ITEMS];
    {
        int32_t yc = start_y + DOCK_ICON_BASE / 2;
        for (int i = 0; i < g_dock_nItems; i++) {
            centers_y[i] = yc;
            yc += DOCK_ICON_BASE + DOCK_ICON_GAP;
        }
    }

    /* Update magnification based on cursor Y distance from each icon center.
     * Only magnify when cursor is over the dock strip. */
    int in_dock = (cur_x >= dock_x0);
    for (int i = 0; i < g_dock_nItems; i++) {
        int32_t dist = cur_y - centers_y[i];
        if (dist < 0) dist = -dist;
        int32_t tgt = 256;
        if (in_dock && dist < DOCK_INFLUENCE) {
            int32_t factor = (DOCK_INFLUENCE - dist) * 256 / DOCK_INFLUENCE;
            tgt = 256 + (int32_t)(((int64_t)(DOCK_MAG_MAX_Q8 - 256) * factor) >> 8);
            if (tgt > DOCK_MAG_MAX_Q8) tgt = DOCK_MAG_MAX_Q8;
        }
        g_dock_scale_tgt[i] = tgt;
        int32_t diff2 = tgt - g_dock_scale_cur[i];
        g_dock_scale_cur[i] += (diff2 * 6) >> 4;
        if (g_dock_scale_cur[i] < 256) g_dock_scale_cur[i] = 256;
        if (g_dock_scale_cur[i] > DOCK_MAG_MAX_Q8) g_dock_scale_cur[i] = DOCK_MAG_MAX_Q8;
    }

    /* Second pass: compute actual rendered positions with magnified sizes.
     * Icons expand leftward as they magnify (stay right-anchored to dock edge). */
    int32_t total_mag = 0;
    for (int i = 0; i < g_dock_nItems; i++) {
        int32_t sz = (DOCK_ICON_BASE * g_dock_scale_cur[i]) >> 8;
        if (sz < 16) sz = 16;
        total_mag += sz + DOCK_ICON_GAP;
    }
    total_mag -= DOCK_ICON_GAP;

    /* Start Y: center the magnified column in the available area */
    int32_t py;
    if (total_mag < avail_h)
        py = PANEL_H + 8 + (avail_h - total_mag) / 2;
    else
        py = PANEL_H + 8;
    ...
    for (int i = 0; i < g_dock_nItems; i++) {
        ...
        /* Icons grow leftward from the right edge */
        int32_t extra_w = sz - DOCK_ICON_BASE;
        if (extra_w < 0) extra_w = 0;
        int32_t bounce_off = dock_bounce_offset(i, now);
        /* Horizontal: right-anchored, growing left */
        int32_t tx = dock_x0 + (DOCK_W - sz) / 2 - bounce_off;
        if (tx < dock_x0 - DOCK_ICON_BASE) tx = dock_x0 - DOCK_ICON_BASE;
        ...
        int hit = (cur_x >= tx && cur_x < tx + sz &&
                   cur_y >= ty && cur_y < ty + sz);
        if (hit) hovered_item = i;

        py += sz + DOCK_ICON_GAP;
    }

    /* Draw the dock strip background FIRST */
    fill_rect(buf, bw, bh, stride, dock_x0, PANEL_H, DOCK_W, (int32_t)bh - PANEL_H,
              COL_PANEL);
    fill_rect(buf, bw, bh, stride, dock_x0, PANEL_H, 1, (int32_t)bh - PANEL_H,
              COL_BORDER);

    /* Draw icons back-to-front */
    for (int i = 0; i < g_dock_nItems; i++) {
        int32_t tx = g_dock_rects[i].x;
        int32_t ty = g_dock_rects[i].y;
        int32_t sz = g_dock_rects[i].w;
        int hovered = (i == hovered_item);
        int is_open = (g_folder_open == i);

        render_dock_tile(buf, bw, bh, stride, tx, ty, sz,
                         g_dock_items[i].kind, g_dock_items[i].idx, hovered);

        /* Open folder indicator: accent dot on left edge of icon */
        if (is_open) {
            int32_t dot_x = tx - 5;
            int32_t dot_y = ty + sz / 2 - 3;
            fill_round_rect(buf, bw, bh, stride, dot_x, dot_y, 3, 6, 2, COL_ACCENT);
        }

        /* Tooltip label for the hovered icon -- pops to the LEFT */
        if (hovered) {
            const char *lab;
            /* BUG1 FIX: bounds-check idx before using it as array index */
            if (g_dock_items[i].kind == DITEM_APP) {
                int ai2 = g_dock_items[i].idx;
                lab = (ai2 >= 0 && ai2 < DOCK_NLOOSE_APPS) ? k_dock_apps[ai2].label : "?";
            } else {
                int fi2 = g_dock_items[i].idx;
                lab = (fi2 >= 0 && fi2 < DOCK_NFOLDERS) ? k_folders[fi2].label : "?";
            }
            int32_t llen = (int32_t)k_strlen(lab);
            int32_t lw = llen * FONT_W + 10;
            int32_t lx = tx - lw - 6;  /* to the LEFT of the icon */
            int32_t ly = ty + (sz - DOCK_LABEL_H) / 2;
            if (lx < 4) lx = 4;
            if (ly < PANEL_H + 2) ly = PANEL_H + 2;
            /* Tooltip background */
            fill_round_rect(buf, bw, bh, stride, lx, ly, lw, DOCK_LABEL_H - 2, 4, 0xFF2C2C2Eu);
            stroke_rect(buf, bw, bh, stride, lx, ly, lw, DOCK_LABEL_H - 2, COL_BORDER);
            font_draw_string(buf, (int)stride, (int)bw, (int)bh,
                             lx + 5, ly + (DOCK_LABEL_H - 2 - FONT_H) / 2, lab, COL_TEXT);
        }
    }

    /* Render open folder popover (on top of icons) */
    if (g_folder_open >= 0 && g_folder_open < g_dock_nItems) {
        /* Advance popover animation */
        long pdt = now - g_popover_anim_start;
        if (pdt < 0) pdt = 0;
        int32_t panim = (POPOVER_ANIM_MS > 0) ? (int32_t)((pdt * 256) / POPOVER_ANIM_MS) : 256;
        if (panim > 256) panim = 256;
        g_popover_anim = panim;

        /* For right-side dock: folder popover opens to the LEFT.
         * Pass icon_cx = left edge of icon so the popover positions leftward. */
        int32_t icx = g_dock_rects[g_folder_open].x;   /* left edge of icon */
        int32_t icy = g_dock_rects[g_folder_open].y + g_dock_rects[g_folder_open].h / 2;
        render_folder_popover(buf, bw, bh, stride, g_folder_open, icx, icy, panim, now);
    }
}

/* ------------------------------------------------------------------ *
 *  M7: handle_dock_click() -- returns 1 if click was consumed        *
 * ------------------------------------------------------------------ */
static int handle_dock_click(int32_t cx, int32_t cy, long now) {
    /* If a folder popover is open, check if click is inside the popover.
     * Popover geometry MUST match render_folder_popover exactly. */
    if (g_folder_open >= 0 && g_folder_open < g_dock_nItems) {
        int fidx = g_dock_items[g_folder_open].idx;
        /* BUG1 FIX: bounds-check fidx before using as k_folders index */
        if (fidx < 0 || fidx >= DOCK_NFOLDERS) { g_folder_open = -1; return 1; }
        int nmembers = k_folders[fidx].count;
        int cols = 3;
        int32_t cell = 52;
        int32_t pad  = 8;
        int32_t box_w = cols * (cell + pad) + pad;
        int32_t box_h = ((nmembers + cols - 1) / cols) * (cell + pad) + pad + FONT_H + 4;
        /* Right-side dock: popover is to the LEFT of the icon */
        int32_t icon_cx  = g_dock_rects[g_folder_open].x;  /* left edge of icon */
        int32_t icon_top = g_dock_rects[g_folder_open].y + g_dock_rects[g_folder_open].h / 2;
        int32_t bx = icon_cx - box_w - 8;
        int32_t by = icon_top - box_h / 2;
        if (bx < 4) bx = 4;
        if (by < PANEL_H + 4) by = PANEL_H + 4;
        if (by + box_h > (int32_t)g_fb_h - 4) by = (int32_t)g_fb_h - 4 - box_h;

        if (cx >= bx && cx < bx + box_w && cy >= by && cy < by + box_h) {
            /* Determine which member was clicked */
            int32_t inner_x = cx - bx - pad;
            int32_t inner_y = cy - (by + FONT_H + 4);
            if (inner_y >= 0) {
                int col = inner_x / (cell + pad);
                int row = inner_y / (cell + pad);
                int m   = row * cols + col;
                if (m >= 0 && m < nmembers) {
                    int ai = k_folders[fidx].members[m];
                    if (ai >= 0 && ai < DOCK_NLOOSE_APPS) {
                        print("[DOCK] launch ");
                        print(k_dock_apps[ai].path);
                        print("\n");
                        syscall(SYS_SPAWN, (long)k_dock_apps[ai].path, 0, 0);
                        /* Bounce the folder icon */
                        g_dock_bounce[g_folder_open].active = 1;
                        g_dock_bounce[g_folder_open].start_ms = now;
                    }
                }
            }
            g_folder_open = -1;
            ...
            /* Click outside popover: collapse */
            g_folder_open = -1;
            return 1;
        }
    }

    /* Hit-test dock icon tiles */
    for (int i = 0; i < g_dock_nItems; i++) {
        int32_t tx = g_dock_rects[i].x;
        int32_t ty = g_dock_rects[i].y;
        int32_t sz = g_dock_rects[i].w;
        if (cx >= tx && cx < tx + sz && cy >= ty && cy < ty + sz) {
            if (g_dock_items[i].kind == DITEM_FOLDER) {
                /* Toggle folder popover */
                if (g_folder_open == i) {
                    g_folder_open = -1;
                } else {
                    g_folder_open = i;
                    g_popover_anim = 0;
                    g_popover_anim_start = now;
                }
            } else {
                int ai = g_dock_items[i].idx;
                /* BUG1 FIX: guard ai before using as array index */
                if (ai < 0 || ai >= DOCK_NLOOSE_APPS) { return 1; }
                print("[DOCK] launch ");
                print(k_dock_apps[ai].path);
                print("\n");
                syscall(SYS_SPAWN, (long)k_dock_apps[ai].path, 0, 0);
                /* Bounce the icon */
                g_dock_bounce[i].active = 1;
                g_dock_bounce[i].start_ms = now;
...
/* font extern + FONT_W/H already declared above for M7 dock code */
 ...
 * BUG1 FIX: sc6(SYS_MSGGET) returns a long.  It must be checked for negative
 * (error) BEFORE being cast to int32_t, to prevent a sign-extension mismatch
 * where a valid positive qid with the int32 sign bit set (> 0x7FFFFFFF) would
 * be stored as a negative int32 and later passed to SYS_MSGSND.  The callers
 * all guard on `< 0` so a negative return safely skips the send. */
static int32_t client_reply_qid(window_t *win) {
    ...
    if (qid < 0) return (int32_t)qid;          /* error — do not cache */
    if (qid > 0x7FFFFFFF) return -1;           /* won't fit in int32 safely — treat as error */
    win->reply_qid = (int32_t)qid;
    return (int32_t)qid;
}
...
/* Right-dock geometry helpers */
static int32_t dock_left(void)             { return (int32_t)g_fb_w - DOCK_W; }
/* Legacy helpers kept so animate-to-taskbar code still compiles (unused path). */
static int32_t dock_top(uint32_t h)        { (void)h; return (int32_t)g_fb_h; } /* no bottom bar */
static int32_t launcher_x(void)            { return dock_left() + DOCK_STRIP_INSET; }
static int32_t launcher_y(uint32_t h)      { (void)h; return PANEL_H + 8; }
static int32_t taskbtn_x(int idx)          { (void)idx; return dock_left(); }
static int32_t taskbtn_y(uint32_t h)       { (void)h; return PANEL_H + 8; }
...
/* Work area: panel at top, right-side dock on the right, bottom is free. */
static int32_t work_x0(void)        { return WORK_MARGIN; }
static int32_t work_y0(void)        { return PANEL_H + WORK_MARGIN; }
/* Right boundary excludes the dock strip */
static int32_t work_x1(void)        { return (int32_t)g_fb_w - DOCK_W - WORK_MARGIN; }
/* Bottom boundary is now the full screen bottom (no bottom dock) */
static int32_t work_y1(void)        { return (int32_t)g_fb_h - WORK_MARGIN; }
...
static void render_window_static(uint32_t *buf, uint32_t w, uint32_t h, uint32_t stride,
    ...
    int32_t clip_y1 = (int32_t)h;  /* no bottom dock; full height below panel */
...
static void render_window_anim(uint32_t *buf, uint32_t w, uint32_t h, uint32_t stride,
    ...
    int32_t ch = (int32_t)win->h;
    int32_t clip_y0 = PANEL_H;
    int32_t clip_y1 = (int32_t)h;  /* no bottom dock */
...
static void composite(uint32_t *buf, uint32_t w, uint32_t h, uint32_t stride,
    ...
    /* M7: macOS-style dock icon strip (right-side vertical dock) */
    /* [DBG] disabled for debug - crash isolated here */
    print("[DBG] G\n");
    if (0) render_dock_icons(buf, w, h, stride, cursor_x, cursor_y, now);
    print("[DBG] H\n");
 ...
 * Right boundary excludes the dock strip; bottom is now the full screen. */
static void clamp_window(window_t *win) {
    int32_t min_y = PANEL_H + 4;
    int32_t max_y = (int32_t)g_fb_h - TITLEBAR_H - 8;
    /* right edge: keep titlebar at least partially visible left of the dock */
    int32_t max_x = dock_left() - (int32_t)win->w / 3;
    if (max_x < 4) max_x = 4;
...
static void send_pointer_to_focus(void) {
    ...
    window_t *win = &g_windows[slot];
    int32_t qid = client_reply_qid(win);
    if (qid < 0) return;                /* guard: never pass negative qid to kernel */
    wl_evt_pointer_t ev;
...
static void pump_input(int32_t fd, int keyboard, uint32_t W, uint32_t H) {
    ...
    for (long i = 0; i < count; i++) {
        ...
        if (e->type == EV_REL) {
            ...
            /* [DBG] Temporary: print every mouse EV_KEY to confirm kernel delivery */
            print("[DBG] btn code="); print_num((long)e->code);
            print(" val="); print_num((long)e->value); print("\n");
            if (bit >= 0) {
...
static int32_t snap_zone_for_cursor(int32_t cx, int32_t cy, uint32_t W, uint32_t H) {
    (void)H;
    if (cy <= PANEL_H + SNAP_EDGE_PX)       return SNAP_MAX;    /* near the top */
    if (cx <= SNAP_EDGE_PX)                  return SNAP_LEFT;
    if (cx >= (int32_t)W - SNAP_EDGE_PX)     return SNAP_RIGHT;
    return SNAP_NONE;
}
...
static void handle_mouse(uint32_t W, uint32_t H) {
    ...
    /* [DBG] Temporary: confirm handle_mouse sees left-button press */
    if (press) { print("[DBG] left press cx="); print_num(cx); print(" cy="); print_num(cy); print("\n"); }
    ...
    /* M7: dock icon strip (checked before legacy dock/taskbar) */
    {
        long now_ms = syscall(SYS_GET_TICKS_MS, 0, 0, 0);
        if (handle_dock_click(cx, cy, now_ms)) {
            g_prev_buttons = g_buttons;
            return;
        }
    }
...
void _start(void) {
    ...
    /* M7: welcome notification toast (fade-in/hold/fade-out over ~3.5s). */
    toast_show("Welcome to AutomationOS M7", 3500);
