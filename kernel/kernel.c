                        ...
                        if (scheduler_init_secondary_cpu(1, ap_apic_id)) {
                            ...
                            ap_spawn_test_kthread();
                            {
                                ...
                                kprintf("[SMP] Brick F2 VERIFY: AP kthread counter "
                                        ...
                                        c0, c1, c1 - c0);
                                /* F3-4 APCURRENT: the kthread (on CPU1) called the real
                                 * process_get_current() and proved it resolves CPU1-local
                                 * (distinct from CPU0's current). The BSP reads the result
                                 * the kthread set and emits the single greppable gate line.
                                 * Proves the per-cpu "current" routing works from a real
                                 * CPU1 context -- the prerequisite for a safe AP syscall. */
