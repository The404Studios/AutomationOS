...
/*
 * net_info_t mirror -- MUST match kernel/include/net.h exactly (same struct
 * used by ping.c / netinfo.c). The kernel's sys_net_info() fills mac/ip/gateway
 * in HOST byte order; ip/gateway are 0 before net_init().
 */
typedef struct {
    u8  mac[6];
    u8  _pad[2];
