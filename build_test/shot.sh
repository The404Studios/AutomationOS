...
# Boots headless, waits for the desktop marker, then screendumps the active
# display surface (works with -display none) to screenshots/<out>.png.
set -u
...
QPID=$!
# wait (bounded) for the desktop to come up, then let it paint a steady frame
...
if [ -f /tmp/${OUT}.ppm ] && [ ! -f /tmp/${OUT}.png ]; then
  ...
  [ -f /tmp/${OUT}.png ] || { command -v convert >/dev/null && convert /tmp/${OUT}.ppm /tmp/${OUT}.png 2>/dev/null; }
fi
if [ -f /tmp/${OUT}.png ]; then cp -f /tmp/${OUT}.png screenshots/${OUT}.png; echo "SAVED screenshots/${OUT}.png ($(stat -c%s screenshots/${OUT}.png) B)"; else echo "NO PNG (converter missing? ppm=$(ls -la /tmp/${OUT}.ppm 2>/dev/null))"; fi
